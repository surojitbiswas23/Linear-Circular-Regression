...
data<-read.csv("daily_eth_stats_2021_with_avg.csv")
lf=603# 1st jan 2018
rt= 814# 31st jult 2018
...
abab=(data$High[lf:rt]/data$Low[lf:rt])*(data$Close[lf:rt]-data$Open[lf:rt]) 
cl_low=1/abab
...
# pdf(file="rose_plot_max_eth_for_regression.pdf")
...
for (j in 1:it) {
  ...
  while (conv > 0) {
    ...
    op = optim(di, fn, lower = c(0, -10, 0),
               upper = c(2 * pi, 10, 100), method = "L-BFGS-B", hessian = TRUE)
  ...
  cat('standard error=',standard_errors, 'perpendicular sum',sum1,'\n')
  # 
  ...
  #pdf(file="true_data_plot_eth.pdf")
  #postscript(file="true_data_plot_eth.eps")
  ...
  #pdf(file="predicted_data_plot_eth.pdf")
  #postscript(file="predicted_data_plot_eth.eps")
  ...
  #pdf(file="error_plot_eth.pdf")
  #postscript(file="error_plot_eth.eps")
  ...
  #pdf(file="QQ_plot_dataset_eth.pdf")
  #postscript(file="QQ_plot_dataset_eth.eps")
