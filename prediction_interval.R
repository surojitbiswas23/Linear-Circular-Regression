...
ty_org=(Arg(beta0 * (xnew - beta1) / (xnew- Conj(beta1))) %% (2 * pi)+ 
                     rvonmises(1,mu=0,rho)%% (2 * pi)) %% (2 * pi)
...
cover_ci<-function(q)
{
 ...
 # mat=matrix(0,nrow = itrn,ncol = 2)
  ...
  for (j in 1:boot) {
    ...
    th_prd1 =numeric(0)
    ty= (yhat0 +sample(er0,n,replace = T))%% (2 * pi)
    ...
    yhat_boot[j]= (Arg(beta0fb * (xnew - beta1fb) / (xnew- Conj(beta1fb))) %% (2 * pi)+
                                 sample(er0,1,replace = T))%% (2 * pi)
    
    yhat_boot[j]= (Arg(beta0fb * (xnew - beta1fb) / (xnew- Conj(beta1fb))) %% (2 * pi)+sample(er0,1,replace = T))%% (2 * pi)
...
write.csv(x = res_matrix,file = 'data_matrix_of_prediction_probability_von_git.csv')

cov_pro=read.csv('data_matrix_of_prediction_probability_von_git.csv')
